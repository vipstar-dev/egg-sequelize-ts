...
module.exports = app => {
  ...
  function loadDatabase(config = {}) {
    ...
    // todo: https://github.com/eggjs/egg-sequelize/commit/263282739cafb22501069afe3304a8e2a76dbd7c support connection uri
    ...
    if (model[delegate[len - 1]]) {
      throw new Error(`[egg-sequelize-ts] app[${config.delegate}] is already defined`);
    }
    ...
    app.loader.loadToApp(modelDir, target, {
      ...
      filter(model) {
        // 使用findAll 替换sequelize，seq-ts在执行addModels才会有 sequelize
        if (!model || !model.findAll) return false;
        models.push(model);
        return true;
      },
    });
